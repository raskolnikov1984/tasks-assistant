...
export function ProjectView({ project, onBack }: ProjectViewProps) {
  ...
  function openCreateModal(status: TaskStatus | null = null, dateOverride?: string) {
    setEditingTask(null);
  ...
  async function handleSubmitTask(e: React.FormEvent) {
    ...
    if (editingTask) {
      await api.updateTask(editingTask.id, title, description, priority, dueDate, dueTime || null);
    } else {
      const status = selectedStatus || 'pending';
      await api.createTask(project.id, title, description, priority, dueDate, dueTime || null);
  ...
  return (
    <div className="project-view">
      ...
      {viewMode === 'board' ? (
        ...
        <CalendarView tasks={tasks} onEditTask={openEditModal} onCreateTask={handleCreateFromCalendar} />
      )}

      {showModal && (
        <div className="modal-overlay">
          <div className="modal">
            <h2>{editingTask ? 'Edit Task' : `Create Task${selectedStatus ? ` → ${columns.find(c => c.id === selectedStatus)?.title}` : ''}`}</h2>
            <form onSubmit={handleSubmitTask}>
              ...
              <div className="modal-actions">
                ...
                <button type="submit" className="btn-primary">{editingTask ? 'Save' : 'Create'}</button>
              </div>
            </form>
          </div>
        </div>
